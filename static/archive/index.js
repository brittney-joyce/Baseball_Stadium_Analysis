...
d3.csv("data/Team_Data.csv", function(err, csvdata) {
    ...
    var yLinearScale = d3.scaleLinear()
        .domain([0, d3.max(csvdata, d => d.atten2017)])
        .range([height, 0]);

    var xLinearScale = d3.scaleLinear()
        .domain([35, d3.max(csvdata, d => d.rev2017)])
        .range([0, width]);
    ...
    .enter()
    .append("circle")
    .attr("cx", d => xLinearScale(d.rev2017))
    .attr("cy", d => yLinearScale(d.atten2017))
    .attr("r", "15")
    .attr("fill", "blue")
    .attr("opacity", ".5");
    ...
    .offset([80, -60])
    .html(function(d) {
      return (`${d.Abbr}<br> Attendance: ${d.atten2017}<br> Revenue: ${d.rev2017} million`);
    });
